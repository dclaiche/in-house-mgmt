...
class ContactViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        if event_category_id:
            attended_event_filter &= Q(event_participations__event__category_id=event_category_id)
            queryset = queryset.filter(
        ...
        if min_tickets and min_tickets.isdigit():
            ...
            queryset = (
                queryset.annotate(num_tickets_in_range=Count("tickets", filter=internal_ticket_filter))
                .filter(num_tickets_in_range__gte=min_tickets)
                .filter(tickets__ticket_status=TicketStatus.COMPLETED)
            )
        ...
        return queryset.filter(get_contact_visibility_filter(self.request.user)).distinct()
    ...
    def acceptance_rate(self, request, pk=None):
        ...
        qs = (
            TicketAsks.objects.filter(contact=contact)
        ...
        response_data = {
            ticket_type_value: {status.value: 0 for status in TicketAskStatus}
