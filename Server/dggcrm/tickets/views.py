...
class TicketViewSet(viewsets.ModelViewSet):
    ...
    permission_classes = [IsAuthenticated, DjangoModelPermissions, TicketObjectPermission]
    ...
    def get_queryset(self):
        ...
        if priority is not None:
            try:
                ...
                raise APIException(detail="Invalid priority in query", code=status.HTTP_400_BAD_REQUEST) from err
        ...
        return queryset.filter(get_ticket_visibility_filter(self.request.user)).distinct()
    ...
    @action(
        ...
        permission_classes=[IsAuthenticated, TicketObjectPermission, TicketClaimPermission],
    )
